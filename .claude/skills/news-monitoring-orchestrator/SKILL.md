...
| web-designer | web-designer (커스텀) | 대시보드 시각 리디자인 (계약 불변, `src/web/`만) | news-web-design | `src/web/` |
...
>
> web-designer는 dashboard-dev가 기능 표면을 완성한 뒤 **시각 리디자인이 필요할 때만** 단독 서브 에이전트로 호출한다. API 응답 shape·필드명을 절대 바꾸지 않고 `src/web/` 프레젠테이션만 개선하므로, 호출 시 보존해야 할 계약 필드(`importance_score`, `category`, `category_label`, `url`, `title`, `importance_reason`, `source`, `published_at`)를 프롬프트에 명시한다.
